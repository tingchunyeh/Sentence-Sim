...
kb_parsed = pd.DataFrame(columns=['category', 'question', 'default_answer'])
for index in tqdm(kb.index, desc="{Read questions from raw knowledge base data}"):
	...
	kb_parsed = kb_parsed.append({'category': category, 'question':question, 'default_answer':answer}, ignore_index=True)

	cur = kb.loc[index, :].dropna()
	for i in range(12, cur.index[-1]):
		sim_question = pattern.sub("" ,cur.loc[i])
		if not sim_question: continue
		kb_parsed = kb_parsed.append({'category': category, 'question':sim_question, 'default_answer':answer}, ignore_index=True)

print ("output kb.csv ...")
kb_parsed.to_csv("kb_parsed.csv", encoding='utf_8_sig', index=False)
